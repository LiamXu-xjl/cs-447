...
Original file is located at
    https://colab.research.google.com/drive/1-W-eVr4y015HixpWp3p8pIkbZBzLqsoi
...
class TextDataset(data.Dataset):
    def __init__(self, examples, split, ixtoword=None, wordtoix=None, THRESHOLD=THRESHOLD, MAX_LEN=MAX_LEN):
        ...
        self.vocab_size = len(self.wordtoix.keys()) ##### TODO : set this to the number of words in your vocabulary
        ...
        for sen_idx, sen in enumerate(self.examples):
            label_interpreted = 1 if self.examples[sen_idx].label == 'Positive' else 0
            self.labels.append(label_interpreted)
            for word in sen.text: 
                if word in self.wordtoix:
                    self.textual_ids.append(self.wordtoix[word])
                else:
                    self.textual_ids.append(1)
        # pass
    def build_dictionary(self): 
        ...
        for sen in self.examples:
            for raw_word in sen.text: 
                word = raw_word.lower()
                count[word] += 1
        filtered_count = filter(lambda x : count[x] >= self.THRESHOLD, count.keys())
        lis = list(filtered_count)
        # Assign idx (starting from 2) to all words having word_freq >= THRESHOLD
        for i, xi in enumerate(lis):
            self.ixtoword[i+2] = xi
            self.wordtoix[xi] = i+2
    ...
    def get_label(self, index):
        ...
        label = torch.LongTensor([1]) if self.examples[index].label == 'positive' else torch.LongTensor([0])
        # print(label)
    ...
    def get_text(self, index):
        ...
        raw_text = self.examples[index].text
        text = []

        # upper_bound = min(len(raw_text),MAX_LEN)
        for i in range(MAX_LEN-1):
            if i < len(raw_text):
                word = raw_text[i]
                if word in self.wordtoix:
                    text.append(self.wordtoix[word])
                else:
                    text.append(1)
            else:
                text.append(0)
        ...
        text_len = torch.LongTensor([len(raw_text)])
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, out_channels, filter_heights, stride, num_classes, dropout, pad_idx):
        ...
        self.conv1 = nn.Conv2d(1, out_channels, (filter_heights[0],embed_size),stride)
        self.conv2 = nn.Conv2d(1, out_channels, (filter_heights[1],embed_size),stride)
        self.conv3 = nn.Conv2d(1, out_channels, (filter_heights[2],embed_size),stride)
    ...
    def forward(self, texts, text_lens):
        ...
        conv_output_1 = self.conv1(emb)
        conv_output_2 = self.conv2(emb)
        conv_output_3 = self.conv3(emb)
        ...
        conv_output_1 = torch.squeeze(conv_output_1,3)
        conv_output_2 = torch.squeeze(conv_output_2,3)
        conv_output_3 = torch.squeeze(conv_output_3,3)
        
        #   Apply non-linearity on it (F.relu() is a commonly used one. Feel free to try others)
        conv_output_1 = F.relu(conv_output_1)
        conv_output_2 = F.relu(conv_output_2)
        conv_output_3 = F.relu(conv_output_3)
        
        #   Take the max value across last dimension to have shape [batch_size, out_channels]
        conv_output_1 = torch.max(conv_output_1, 2).values
        conv_output_2 = torch.max(conv_output_2, 2).values
        conv_output_3 = torch.max(conv_output_3, 2).values       

        # Concatenate (torch.cat) outputs from all your cnns [batch_size, (out_channels*num_of_cnn_layers)]
        conv_output = torch.cat((conv_output_1, conv_output_2, conv_output_3), 1)
...
if __name__=='__main__':
    ...
    DROPOUT = 0
    OUT_CHANNELS = 64
    FILTER_HEIGHTS = [1,4,7] # [3 different filter sizes - unigram, bigram, trigram in this case. Feel free to try other n-grams as well]
    STRIDE = 1
...
if __name__=='__main__':    
    LEARNING_RATE = 8e-4 # Feel free to try other learning rates
...
class RNN(nn.Module):
    ...
    def forward(self, texts, text_lens):
        ...
        emb = nn.utils.rnn.pack_padded_sequence(emb, text_lens.shape[0]*[MAX_LEN])
        ...
        out_rnn = torch.cat((h_n_f, h_n_b), 1)
